...
min16float4 TraceIndirect(Texture3D<float> txSDF, Texture3D<float3> txIrradiance, RayDesc ray, float falloff)
{
	const uint n = 32;
	const float nInv = 1.0 / n;
	const float rad = 1.0 - nInv; // Hemispherical factor (self occlusion correction)
	...
	for (uint i = 0; i < n; ++i)
	{
		...
		//if (r < 1e-2)
		{
			irradiance += min16float3(txIrradiance.SampleLevel(g_sampler, uvw, 0.0));
		}
	}

	const float ao = saturate(1.0 - occ * nInv);
	irradiance *= min16float(1.0 - ao) / n;

	return min16float4(irradiance, t);
}
